...
\subsection*{$P_{59}$ 通过最大间隔法将分类问题等价为 }
\begin{align*}
\tag{2.2.3}
\max_{\omega, b} \qquad & \frac{2}{\| \omega \|} \\
\tag{2.2.4}
s.t. \qquad & \forall i \in \{i| y_i = 1\}: (\omega \dot x_i) + b \ge 1 \\
\tag{2.2.5}
& \forall i \in \{i| y_i = -1\}: (\omega \dot x_i) + b \le -1 \\
\end{align*}
或者：
\begin{align*}
\tag{2.2.6}
\min_{\omega, b} \qquad & \frac{2}{\| \omega \|} \\
\tag{2.2.4}
s.t. \qquad & \forall i \in \{i| y_i = 1\}: (\omega \dot x_i) + b \ge 1 \\
\tag{2.2.5}
& \forall i \in \{i| y_i = -1\}: (\omega \dot x_i) + b \le -1 \\
